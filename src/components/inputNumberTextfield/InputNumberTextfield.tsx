...
const InputNumberTextfield = ({ name, value, onChange }: Props) => {
  const preventNoneValue = (func: (args: number) => any, args: number) => {
    if (!args) {
      args = 0;
      //TODO think of better way to define this
    } else {
      func(args);
    }
  };
  ...
  return (
    <div className="gap-4">
      ...
      <input
        ...
        type="number"
        value={value}
        onChange={(e) => preventNoneValue(onChange, parseInt(e.target.value))}
      />
    </div>
  );
};
